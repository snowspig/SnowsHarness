...
Run `./deploy.sh` (Linux/macOS) or `.\deploy.ps1` (Windows) interactively — you'll be prompted to choose between **full** or **mini** mode:

```bash
./deploy.sh
# Select deployment mode:
#   1) full  - Complete rules with detailed examples (baseline)
#   2) mini  - Compressed rules, ~17% token savings (daily dev)
...
./deploy.sh --mini       # Deploy with compressed rules
./deploy.sh --full       # Deploy with full rules
...
| ------------------ | ------------------------------------- | ------------ | --------------------------------------- |
| **full**           | Complete rules with detailed examples | Baseline     | Learning, need detailed guidance        |
| **mini** (default) | Compressed rules, same functionality  | ~17% savings | Daily development, faster conversations |

Switch modes after deploy:

```bash
node ~/.claude/hooks/switch-harness-mode.js mini
node ~/.claude/hooks/switch-harness-mode.js full
```
...
./deploy.sh --mini       # 部署精简规则（节省 ~17% token）
```

运行 `./deploy.sh`（Linux/macOS）或 `.\deploy.ps1`（Windows）进入交互式部署 — 系统会提示你选择模式：

```bash
./deploy.sh
# Select deployment mode:
#   1) full  - 完整规则和详细示例 (基线)
#   2) mini  - 压缩规则，节省 ~17% token (日常开发)
# Choose mode [1/2, default: 2]:
```
...
# Linux/macOS
./deploy.sh --dry-run    # 预览变更
./deploy.sh --force      # 无提示部署（默认 mini）
./deploy.sh --mini       # 部署精简规则
./deploy.sh --full       # 部署完整规则
...
| 模式             | 说明               | Token 开销 | 适用场景           |
| ---------------- | ------------------ | ---------- | ------------------ |
| **full**         | 完整规则和详细示例 | 基线       | 学习、需要详细指导 |
| **mini**（默认） | 压缩规则，功能相同 | 节省 ~17%  | 日常开发、快速对话 |
